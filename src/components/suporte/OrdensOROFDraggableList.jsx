...
import OrdemOROFCard from './OrdemOROFCard';

export default function OrdensOROFDraggableList({ ops, itensOROF, onAdicionar, onEditar, onExcluir, onMover, loadingItem }) {
  const queryClient = useQueryClient();
  const [localOrderIds, setLocalOrderIds] = useState([]);
  const [salvandoOrdem, setSalvandoOrdem] = useState(false);

  const opsIds = ops.map(o => o.id).join(',');
  useEffect(() => {
    if (ops.length > 0) {
      setLocalOrderIds(ops.map(o => o.id));
    }
  }, [opsIds]);

  const orderedOps = localOrderIds.length > 0
    ? [
        ...localOrderIds.map(id => ops.find(o => o.id === id)).filter(Boolean),
        ...ops.filter(o => !localOrderIds.includes(o.id))
      ]
    : ops;
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      const updates = newOrderIds.map((id, idx) => ({ id, ordem_visualizacao: idx }));
      await base44.entities.OrdemProducao.bulkUpdate(updates);
      queryClient.invalidateQueries({ queryKey: ['ops-orof'] });
    } catch (error) {
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="ordens-orof">
        {(provided) => (
