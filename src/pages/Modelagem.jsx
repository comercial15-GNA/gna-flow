...
export default function Modelagem() {
  ...
  const { data: itens = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const items = await base44.entities.ItemOP.filter({ etapa_atual: 'modelagem' });
  ...
  const confirmarRetorno = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['itens-modelagem'] });
      queryClient.invalidateQueries({ queryKey: ['ops-all'] });
      toast.success('Item retornado para Engenharia');
      setRetornarDialogOpen(false);
    } catch (error) {
  ...
  const gerarRelatorio = () => {
    ...
    link.download = `relatorio_modelagem_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`;
    link.click();
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {opsComItens.map(({ op, itens: itensOP }) => {
            ...
            return (
              <div key={op.id} className="bg-white rounded-xl border-2 border-yellow-200 shadow-sm overflow-hidden">
                <button
                  onClick={() => toggleOP(op.id)}
                  className="w-full bg-yellow-50 border-b border-yellow-200 p-4 hover:bg-yellow-100 transition-colors text-left"
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-3">
                      {itensOP.map((item) => {
                        ...
                        return (
                          <div key={item.id} className={`rounded-lg border-2 p-4 ${item.alerta_retorno ? 'bg-red-50 border-red-500' : 'bg-yellow-50 border-yellow-300'} ${item.iniciado ? 'ring-2 ring-blue-500' : ''}`}>
                            ...
                            <ItemOPActions item={item} onUpdate={() => queryClient.invalidateQueries({ queryKey: ['itens-modelagem'] })} />
