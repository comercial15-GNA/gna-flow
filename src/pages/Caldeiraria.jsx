...
export default function Caldeiraria() {
  ...
  const { data: itens = [], isLoading } = useQuery({
    queryKey: ['itens-caldeiraria'],
    queryFn: async () => {
      const items = await base44.entities.ItemOP.filter({ etapa_atual: 'caldeiraria' });
  ...
  const gerarRelatorio = () => {
    ...
    link.download = `relatorio_caldeiraria_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`;
    link.click();
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Caldeiraria</h1>
            <p className="text-slate-500">Itens em processo de caldeiraria</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {opsComItens.map(({ op, itens: itensOP }) => {
            ...
            return (
              <div key={op.id} className="bg-white rounded-xl border-2 border-amber-200 shadow-sm overflow-hidden">
                <button
                  onClick={() => toggleOP(op.id)}
                  className="w-full bg-amber-50 border-b border-amber-200 p-4 hover:bg-amber-100 transition-colors text-left"
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-3">
                      {itensOP.map((item) => {
                        ...
                        return (
                          <div key={item.id} className={`bg-amber-50 rounded-lg border-2 ${item.alerta_retorno ? 'border-red-500 bg-red-50' : 'border-amber-300'} p-4 ${item.iniciado ? 'ring-2 ring-blue-500' : ''}`}>
                            ...
                            <ItemOPActions item={item} onUpdate={() => queryClient.invalidateQueries({ queryKey: ['itens-caldeiraria'] })} />
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleRetornar(item, 'usinagem')}
                                disabled={loadingItem === item.id}
                                className="text-amber-600 border-amber-300 hover:bg-amber-50"
                              >
                                <RotateCcw className="w-3 h-3 mr-1" />
