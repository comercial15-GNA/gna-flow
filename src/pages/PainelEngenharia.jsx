...
import { Cog, AlertTriangle, Calendar, Package, User } from 'lucide-react';
import { format, isBefore, startOfDay } from 'date-fns';
import { ptBR } from 'date-fns/locale';

export default function PainelEngenharia() {
  const { data: itens = [], isLoading } = useQuery({
    queryKey: ['itens-painel-engenharia'],
    queryFn: async () => {
      const items = await base44.entities.ItemOP.filter({ etapa_atual: 'engenharia' });
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-6">
      ...
      <div className="bg-slate-800 rounded-xl p-6 mb-6 border-b-4 border-green-500">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white tracking-tight">ENGENHARIA</h1>
              <p className="text-green-400 text-lg">Próximas 30 Ordens de Produção</p>
            </div>
          </div>
