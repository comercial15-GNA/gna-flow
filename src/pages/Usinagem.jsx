...
export default function Usinagem() {
  ...
  const { data: itens = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const items = await base44.entities.ItemOP.filter({ etapa_atual: 'usinagem' });
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {opsComItens.map(({ op, itens: itensOP }) => {
            ...
            return (
              <div key={op.id} className="bg-white rounded-xl border-2 border-cyan-200 shadow-sm overflow-hidden">
                <button
                  onClick={() => toggleOP(op.id)}
                  className="w-full bg-cyan-50 border-b border-cyan-200 p-4 hover:bg-cyan-100 transition-colors text-left"
                >
                      ...
                      {itensOP.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="bg-cyan-50 rounded-lg border-2 border-cyan-300 p-4">
                            ...
                            <ItemOPActions item={item} onUpdate={() => queryClient.invalidateQueries({ queryKey: ['itens-usinagem'] })} />
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleRetornar(item, 'fundicao')}
                                disabled={loadingItem === item.id}
