      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {opsComItens.map(({ op, itens: itensOP }) => {
            ...
            return (
              <div key={op.id} className="bg-white rounded-xl border-2 border-violet-200 shadow-sm overflow-hidden">
                <button
                      ...
                      {itensOP.map((item) => {
                        ...
                        return (
                          <div key={item.id} className={`bg-violet-50 rounded-lg border-2 border-violet-300 p-4 ${item.iniciado ? 'ring-2 ring-blue-500' : ''}`}>
                            ...
                            <ItemOPActions item={item} onUpdate={() => queryClient.invalidateQueries({ queryKey: ['itens-montagem'] })} />
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              {ETAPAS_RETORNO.map((etapa) => (
                                <Button
                                  ...
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleRetornar(item, etapa.value)}
                                  disabled={loadingItem === item.id}
